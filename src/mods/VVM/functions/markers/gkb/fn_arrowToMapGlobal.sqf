/* Function: GKB_fnc_arrowToMapGlobal - Author: Gekkibi - Licence: http://creativecommons.org/licenses/by-nc-sa/3.0/
 * Description:	Draws an arrow globally to the map. Optional text inside the arrow.
 * Dependencies: GKB_fnc_lineToMapGlobal, GKB_fnc_textToMapGlobal (optional).
 * Example: _markerArray = [[2500,3000],[45,600,50,5],["ColorGreen",1],["advance!","front","ColorBlue",1]] call GKB_fnc_arrowToMapGlobal;
 * Returns: Marker names in an array.
 * Parameters:
 ...
 * 	4th	Array (optional)	Arrow text (string), text offset position ("front", "back", "center"), text color (string and text alpha (number)
 */
...
if (!isNil "_textArray") then {
	...
	_textLength = 0;
	{	_symbol = toString [_x] call VVM_fnc_textToMapFont;
	...
	if (_direction / 360 mod 1 <= 0.5) then {
		...
		call {
			if (_textPosition == "front") exitWith { _textOffset = _width + _textLength; };
			if (_textPosition == "back") exitWith { _textOffset = _length; };
			if (_textPosition == "center") exitWith { _textOffset = (_length + _textLength) / 2; };
		};
	} else {
		_textDirection = -90 - _direction;
		call {
			if (_textPosition == "front") exitWith { _textOffset = _width; };
			if (_textPosition == "back") exitWith { _textOffset = _length - _textLength; };
			if (_textPosition == "center") exitWith { _textOffset = (_length - _textLength) / 2; };
		};
	};
	_markerTextArray = [
	...
	] call VVM_fnc_textToMapGlobal;
} else { _markerTextArray = []; };
{ _markerArray set [count _markerArray, _x]; } forEach _markerTextArray;
_markerArray;
