/* Function: GKB_fnc_writeToMap - Author: Gekkibi - Licence: http://creativecommons.org/licenses/by-nc-sa/3.0/
 * Description: Write text locally to the map. Requires GKB_fnc_writeToMapFont.
 * Dependencies: GKB_fnc_writeToMapFont
 * Example: _text = [[1000,5000],[-20,2],["suspected enemy minefield!","ColorRed",0.9]] call GKB_fnc_writeToMap;
 * Returns: Marker names in an array
 * Parameters:
 * 	1st	Numbers in array	Position of the text
 * 	2nd	Numbers in array	Rotation and the size of the text. Negative rotation rotates the text clockwise
...
if (local player) then {
	...
	for "_y" from 1 to count _text do {
		_symbol = toString [_text select _y - 1] call VVM_fnc_writeToMapFont;
