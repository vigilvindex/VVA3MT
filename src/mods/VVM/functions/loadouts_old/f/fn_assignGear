...
if (_isMan && !_skipCheck) then {
	if (handgunWeapon _unit != "") then {
		private ["_usableMags","_hasMags"];
		_usableMags = getArray(configFile >> "CfgWeapons" >> (handgunWeapon _unit) >> "magazines");
		
		if (count _usableMags > 0) then {
			_hasMags = false;
			{ 
				if (_x in magazines _unit) exitWith {_hasMags = true}; 
			} forEach _usableMags;
			
			if (!_hasMags) then {
				diag_log text format ["[F3] WARNING (fn_assignGear.sqf): Unit %1 %2_%3 has no additional magazines for side-arm '%4'. Expecting %5",_unit,_side,_typeofUnit,handgunWeapon _unit,_usableMags];
				["fn_assignGear.sqf",format["Unit %1 %2_%3 has no additional magazines for side-arm (%4). Expecting %5",_unit,_side,_typeofUnit,handgunWeapon _unit,_usableMags]] call f_fnc_logIssue;
			};
		};
	};
	
	if (primaryWeapon _unit == "") then {
		diag_log text format ["[F3] WARNING (fn_assignGear.sqf): Unit %1 %2_%3 has no primary weapon!",_unit,_side,_typeofUnit];
		["fn_assignGear.sqf",format["Unit %1 %2_%3 has no primary weapon!",_unit,_side,_typeofUnit]] call f_fnc_logIssue;
	} else {
		private ["_usableMags","_hasMags"];
				
		_usableMags = getArray(configFile >> "CfgWeapons" >> (primaryWeapon _unit) >> "magazines");
		...
		if (count _usableMags > 0) then {
			...
			if !_hasMags then {
				...
				diag_log text format ["[F3] WARNING (fn_assignGear.sqf): Unit %1 %2_%3 has no additional magazines for primary weapon '%4'. Expected %5",_unit,_side,_typeofUnit,primaryWeapon _unit,_usableMags];
				_unit addMagazines [(_usableMags select 0),4];
				reload _unit;
			};
		};
	};
	...
	if !(_unit canAdd "ItemMap") then {
		diag_log text format ["[F3] WARNING (assignGear.sqf): Gear for %1 %2_%3 slot full/overfilled.", _unit, _side, _typeofUnit ];
...
if (isNil "_carbine") then { //_carbine should exist unless no faction has been called
	diag_log text format ["[F3] WARNING (fn_assignGear.sqf): Side: %1 has no load-out defined for '%2'", _side, _typeofUnit ];
	["fn_assignGear.sqf",format["Faction: %1 Side: %3 is not defined for '%2'.",_faction,_typeofUnit,_side]] call f_fnc_logIssue;
};