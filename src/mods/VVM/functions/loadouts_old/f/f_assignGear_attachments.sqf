	...
	
	{
		...
		
		{
			if (getNumber(configFile >> "CfgWeapons" >> _x >> "itemInfo" >> "type") > 0) then {
				...
				diag_log text format["[F3] ERROR (f_assignGear_attachments.sqf): Invalid attachment for %1_%2: '%3'",_side,_typeofUnit,_x];
				["f_assignGear_attachments.sqf",format["Invalid attachment for %1_%2: '%3'",_side,_typeofUnit,_x]] call f_fnc_logIssue;
			};
		} forEach _attachment;
...
if (handgunWeapon _unit != "") then {
	...
	// loop trough the attachments and add them to the weapon
	{
		if (isClass (configFile >> "CfgWeapons" >> _x)) then {
			...
			diag_log text format["[F3] ERROR (f_addWeapon.sqf): Invalid weapon for %1_%2: '%3'.",_side,_typeofUnit,_x];
