...
_scope_frontline = ["optic_ACO", "optic_Holosight_blk_F", "optic_MRCO"]; // Standard choice for infantry
_scope_support = ["optic_ACO", "optic_Holosight_blk_F"]; // Support infantry don't get extra options
_scope_dm = ["optic_DMS", "optic_Hamr", "optic_MRCO"]; // Designated Marksman Night
...
_attach_ar = [_attach1, _scope_frontline, _bipod1, _silencer2];
_attach_dm = [_attach1, _scope_dm, _bipod1, _silencerdm];
_attach_sp = [_attach1, _scope_support, _silencer1];
...
_officerpistol = "hgun_ACPC2_F";
_officerpistolmag = "9Rnd_45ACP_Mag";
...
_baseHelmet = ["H_HelmetB_light_black"];
...
_baseGlasses = ["G_Balaclava_TI_G_blk_F"];
...
// Pilot
_pilotUniform = "H_I_HeliPilotCoveralls";
...
switch (_typeOfUnit) do
{
	...
	// LOADOUT: EXPLOSIVES SPECIALIST (DEMO)
	case "exp":
	{
		[_typeOfUnit] call _backpack;
		_unit setUnitTrait ["explosiveSpecialist",1];
