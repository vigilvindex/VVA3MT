/*****************************************************************************
Function name: VVM_fnc_cronJobAdd;
Authors: longbow
License: MIT License
...
Arguments:
	ARRAY [_JOB,_ARGS,_FIRSTRUN,_INTERVAL,_ID]
		...
		_ID - Integer, just job's numeric identifier, use 0 to create a new
			...
			parameters are stored on server in variable VVM_var_cronJob_X,
			where X = _ID.
...
if (isServer) then {
	// debug logging
	if (VVM_var_cronDebug) then
	{
		diag_log format ["VVM_fnc_cronJobAdd: Entering function"];
	};
	...
	if (VVM_var_cronDebug) then
	{
		diag_log format ["VVM_fnc_cronJobAdd: supplied job params: %1",_job];
	};
	...
	if (VVM_var_cronDebug) then
	{
		diag_log format ["VVM_fnc_cronJobAdd: creating/modifying job: %1",_index];
	};
	...
	// means that we put all these jobs to var VVM_var_cronJobsAt_0
	...
	if (VVM_var_cronDebug) then
	{
		diag_log format ["VVM_fnc_cronJobAdd: cron schedule before adding a job: %1",VVM_var_cronSchedule];
	};
	if (!(_first_time in VVM_var_cronSchedule)) then
	{
		// adding to timetable and sorting array to have earliest event
		// always available by (VVM_var_cronSchedule select 0)
		VVM_var_cronSchedule pushBack _first_time;
		VVM_var_cronSchedule sort true;
	};
	if (VVM_var_cronDebug) then
	{
		diag_log format ["VVM_fnc_cronJobAdd: cron schedule after adding a job: %1",VVM_var_cronSchedule];
	};
	// get the list of existing jobs for requested time
	_jobs = missionNamespace getVariable [format["VVM_var_cronJobsAt_%1",_first_time], []];
	_jobs pushBack _index;
	missionNamespace setVariable [format["VVM_var_cronJobsAt_%1", _first_time], _jobs];
	if (VVM_var_cronDebug) then
	{
		diag_log format ["VVM_fnc_cronJobAdd: Exiting function"];
	};
};