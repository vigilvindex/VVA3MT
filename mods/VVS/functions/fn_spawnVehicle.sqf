/*	File: fn_spawnVehicle.sqf
	...
	Description: Spawns the selected vehicle, if a vehicle is already on the spawn point then it deletes the vehicle from the spawn point.
*/
...
if(isNil "_position") exitWith {hint "The spawn point marker doesn't exist?";}; //Make sure the marker exists in a way.
_spCheck = nearestObjects[_position,["landVehicle","Air","Ship"],12] select 0;
if(!isNil "_spCheck") then {deleteVehicle _spCheck;}; //Check to make sure the spawn point doesn't have a vehicle on it, if it does then delete it.
...
if((_cfgInfo select 4) == "Autonomous") then { createVehicleCrew _vehicle; };
...
hint format["You have spawned a %1 at %2",_displayName,getPos _vehicle];
closeDialog 0;
