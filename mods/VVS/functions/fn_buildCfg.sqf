/*	File: fn_buildCfg.sqf
	Author: Bryan "Tonic" Boardwine edited by [TF]def 2015 09 20 (0.4.1)
	Description: Builds our configuration arrays for vehicles to display.
	0: classname 1: scope 2: picture 3: displayname 4: vehicleclass 5: side 6: faction
*/
...
// FOR EXAMPLE, YOU WANT TO ADD CLASS "AIR2" TO AIR VEHICLE SPAWNER. ADD IT ON LINE 83 RIGHT AFTER "rhs_vehclass_helicopter".
		// CARS and trucks
					if ((count VVS_Car == 0) && (_vehicleClass in ["Car","rhs_vehclass_car","rhs_vehclass_truck"])) then { VVS_pre_Car set[count VVS_pre_Car,_className]; };
		// SHIPS and Submarines
					if ((count VVS_Ship == 0) && (_vehicleClass in ["Ship","Submarine"])) then { VVS_pre_Ship set[count VVS_pre_Ship,_className]; };
		// AIR
					if ((count VVS_Air == 0) && (_vehicleClass in ["Air","rhs_vehclass_aircraft","rhs_vehclass_helicopter"])) then { VVS_pre_Air set[count VVS_pre_Air,_className]; };
		...
		// Autonomous and rhs AA
					if ((count VVS_Autonomous == 0) && (_vehicleClass in ["Autonomous","rhs_vehclass_aa"])) then { VVS_pre_Autonomous set[count VVS_pre_Autonomous,_className]; };
		// Support, rhs radar, rhs launcher, rhs targeting
					if ((count VVS_Support == 0) && (_vehicleClass in ["Support","rhs_vehclass_radar","rhs_vehclass_launcher","rhs_vehclass_targeting"])) then { VVS_pre_Support set[count VVS_pre_Support,_className]; };
// <def 2015 09 20
				};
			};
		};
