/*	File: fn_filterType.sqf
	...
	Description: Filters through the incoming request and also checks if the side restriction is enabled.
*/
private["_filter","_ret","_vehicleList"];
_filter = [_this,0,"",[""]] call BIS_fnc_param;
if(_filter == "") exitWith {[]};
_ret = []; //Setup our array.
...
{	_cfgInfo = [_x] call VVS_fnc_cfgInfo;
	if(count _cfgInfo > 0) then	{
		if(VVS_SideOnly) then {
			...
			if(_side == playerSide) then { _ret set[count _ret,_x]; };
		}else{
